...
const UploadDocuments = () => {
  const { next, registrationUser } = React.useContext(DealershipContext);
  ...
  const submitFiles = ({ file }: any, numberDoc: number) => {
    if (file.size > 5242880) {
      ...
      api
        .post(
          `/price/company-upload?userId=${registrationUser.userId}&typeDocumentCompany=${numberDoc}`,
          formData,
          {
            headers: {
  ...
  const removeFile = (file: any) => {
    setLoading(true);
    api
      .delete(`/price/company-upload/${registrationUser.userId}`)
      .then(() => {
        openNotificationWithIcon({
          type: "success",
          title: file.name,
  ...
  const getFiles = () => {
    setLoading(true);
    api
      .get(`/price/company-upload/${registrationUser.userId}`)
      .then(({ data }) => {
